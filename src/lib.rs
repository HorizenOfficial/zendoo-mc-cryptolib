...
fn deserialize_from_file<T: FromBytes>(file_path: &Path, struct_type: &str) -> Option<T>{
...
) -> bool
{
    ...
    //Read vk from file
    let vk = match read_raw_pointer(vk, "sc verification key"){
        Some(vk) => vk,
        None => return false,
    };
