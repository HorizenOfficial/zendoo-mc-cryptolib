...
use cctp_primitives::{
    ...
    proving_system::{
        ...
        verifier::{
            batch_verifier::ZendooBatchVerifier, ceased_sidechain_withdrawal::CSWProofUserInputs,
    ...
    utils::{
        ...
        mht::*,
        poseidon_hash::*,
        serialization::{deserialize_from_buffer, serialize_to_buffer},
    },
};
...
pub(crate) fn get_hex<T: CanonicalSerialize>(
    elem: &T,
...
#[cfg(not(target_os = "windows"))]
fn parse_path<'a>(path: *const u8, path_len: usize) -> &'a Path {
...
pub extern "C" fn zendoo_compute_sc_id(
    ...
    let rs_tx_hash =
        try_get_buffer_constant_size!("tx_hash", tx_hash, UINT_256_SIZE, ret_code, null_mut());
    match compute_sc_id(rs_tx_hash, pos) {
        Ok(sc_id) => Box::into_raw(Box::new(sc_id)),
        Err(e) => {
            *ret_code = CctpErrorCode::HashingError;
            log::error!("{:?}", format!("Error computing sc_id: {:?}", e));
            null_mut()
        }
    }
}
...
pub extern "C" fn zendoo_commitment_tree_add_scc(
    ...
    let rs_pub_key =
        try_get_buffer_constant_size!("pub_key", pub_key, UINT_256_SIZE, ret_code, false);
    let rs_tx_hash =
        try_get_buffer_constant_size!("tx_hash", tx_hash, UINT_256_SIZE, ret_code, false);
    ...
    let rs_custom_fe_conf = try_get_optional_buffer_variable_size!(
        "custom_field_elements_config",
        custom_field_elements_config,
        ret_code,
        false
    );
    let rs_ccd =
        try_get_optional_buffer_variable_size!("custom_creation_data", ccd, ret_code, false);
...
pub extern "C" fn zendoo_commitment_tree_add_fwt(
    ...
    let rs_pub_key =
        try_get_buffer_constant_size!("pub_key", pub_key, UINT_256_SIZE, ret_code, false);
    let rs_mc_return_address = try_get_buffer_constant_size!(
        "mc_return_address",
        mc_return_address,
        UINT_160_SIZE,
        ret_code,
...
pub extern "C" fn zendoo_commitment_tree_add_bwtr(
    ...
    let rs_sc_id = try_read_raw_pointer!("sc_id", sc_id, ret_code, false);
    let rs_mc_dest_addr =
        try_get_buffer_constant_size!("mc_dest_addr", mc_dest_addr, UINT_160_SIZE, ret_code, false);
    let rs_tx_hash =
        try_get_buffer_constant_size!("tx_hash", tx_hash, UINT_256_SIZE, ret_code, false);

    // Read sc_req_data_list
    let rs_sc_req_data =
        try_read_double_raw_pointer!("sc_req_data", sc_req_data, sc_req_data_len, ret_code, false);
...
pub extern "C" fn zendoo_commitment_tree_add_csw(
    ...
    let rs_sc_id = try_read_raw_pointer!("sc_id", sc_id, ret_code, false);
    let rs_pk_hash =
        try_get_buffer_constant_size!("pk_hash", pk_hash, UINT_160_SIZE, ret_code, false);
...
pub extern "C" fn zendoo_commitment_tree_add_cert(
    ...
    let rs_sc_id = try_read_raw_pointer!("sc_id", sc_id, ret_code, false);
    let rs_end_cum_comm_tree_root = try_read_raw_pointer!(
...
pub extern "C" fn zendoo_commitment_tree_get_commitment(
    ...
    match cmt.get_commitment() {
        ...
        None => {
            *ret_code = CctpErrorCode::GenericError;
            log::error!("{:?}", "get_commitment() failed!");
            null_mut()
        }
    }
}
    ...
    match compress_bit_vector(bit_vector, algorithm) {
        ...
        Err(e) => {
            *ret_code = CctpErrorCode::CompressError;
            log::error!("compress_bit_vector() failed: {:?}", e.to_string());
            null_mut()
        }
    }
}
...
pub extern "C" fn zendoo_decompress_bit_vector(
    ...
    let compressed_slice =
        try_get_buffer_variable_size!("compressed_bit_vector", buffer, ret_code, null_mut());
...
pub extern "C" fn zendoo_merkle_root_from_compressed_bytes(
    ...
    let compressed_slice =
        try_get_buffer_variable_size!("compressed bytes", buffer, ret_code, null_mut());
...
pub extern "C" fn zendoo_serialize_field(
    ...
    try_serialize_from_raw_pointer!(
        "field_element",
        field_element,
...
pub extern "C" fn zendoo_deserialize_field(
    ...
    try_deserialize_to_raw_pointer!(
        "field_bytes",
        &(unsafe { &*field_bytes })[..],
        None,
        None,
...
fn _zendoo_init_dlog_keys(max_segment_size: usize, ret_code: &mut CctpErrorCode) -> bool {
    *ret_code = CctpErrorCode::OK;

    match init_dlog_keys(ProvingSystem::Darlin, max_segment_size) {
        Ok(()) => true,
        Err(e) => {
            log::error!("{:?}", format!("Error bootstrapping DLOG keys: {:?}", e));
            *ret_code = CctpErrorCode::GenericError;
            false
        }
    }
...
pub extern "C" fn zendoo_get_proving_system_type(
    ...
    match deserialize_from_buffer::<ProvingSystem>(&[byte], None, None) {
        Ok(ps_type) => ps_type,
        Err(e) => {
            log::error!("Error reading ProvingSystem: {:?}", e);
...
pub extern "C" fn zendoo_serialize_sc_proof(
    ...
    match serialize_to_buffer(sc_proof, Some(compressed)) {
        ...
        Err(e) => {
            log::error!("{:?}", format!("Error serializing proof {:?}", e));
            *ret_code = CctpErrorCode::InvalidValue;
            null_mut()
        }
    }
...
pub extern "C" fn zendoo_deserialize_sc_proof(
    ...
    let sc_proof_bytes =
        try_get_buffer_variable_size!("sc_proof_buffer", sc_proof_bytes, ret_code, null_mut());
    try_deserialize_to_raw_pointer_strict!(
        "sc_proof_bytes",
        sc_proof_bytes,
...
pub extern "C" fn zendoo_get_sc_proof_proving_system_type_from_buffer(
    ...
    let sc_proof_bytes = try_get_buffer_variable_size!(
        "sc_proof_buffer",
        sc_proof_bytes,
        ret_code,
        ProvingSystem::Undefined
...
pub extern "C" fn zendoo_get_sc_proof_proving_system_type_from_file(
    ...
    match cctp_primitives::utils::serialization::read_from_file::<ProvingSystem>(
        proof_path, None, None,
    ) {
        Ok(ps_type) => {
            *ret_code = CctpErrorCode::OK;
            ps_type
...
pub extern "C" fn zendoo_deserialize_sc_vk_from_file(
    ...
    // Deserialize vk
    try_deserialize_to_raw_pointer_from_file!(
        "vk",
        vk_path,
        Some(semantic_checks),
        Some(compressed),
...
pub extern "C" fn zendoo_deserialize_sc_vk(
    ...
    let sc_vk_bytes =
        try_get_buffer_variable_size!("sc_vk_buffer", sc_vk_bytes, ret_code, null_mut());
    try_deserialize_to_raw_pointer_strict!(
        "sc_vk_bytes",
        sc_vk_bytes,
        Some(semantic_checks),
...
pub extern "C" fn zendoo_get_sc_vk_proving_system_type_from_buffer(
    ...
    let sc_vk_bytes = try_get_buffer_variable_size!(
        "sc_vk_buffer",
        sc_vk_bytes,
        ret_code,
        ProvingSystem::Undefined
...
fn get_cert_proof_usr_ins<'a>(
    ...
    let rs_sc_prev_wcert_hash =
        try_read_optional_raw_pointer!("sc_prev_wcert_hash", sc_prev_wcert_hash, ret_code, None);
...
pub extern "C" fn zendoo_verify_certificate_proof(
    ...
    match verify_zendoo_proof(
        ...
        Ok(res) => res,
        Err(e) => {
            log::error!("{:?}", format!("Proof verification failure {:?}", e));
            match e {
...
fn get_csw_proof_usr_ins<'a>(
    ...
    let rs_mc_pk_hash =
        try_get_buffer_constant_size!("mc_pk_hash", mc_pk_hash, UINT_160_SIZE, ret_code, None);

    // Read field element
    let rs_cert_data_hash =
        try_read_optional_raw_pointer!("cert_data_hash", cert_data_hash, ret_code, None);
    let rs_end_cum_comm_tree_root = try_read_raw_pointer!(
        "end_cum_comm_tree_root",
        end_cum_comm_tree_root,
        ret_code,
    ...
    Some(CSWProofUserInputs {
        ...
        cert_data_hash: if rs_cert_data_hash.is_some() {
            rs_cert_data_hash.unwrap()
        } else {
...
pub extern "C" fn zendoo_get_cert_data_hash(
    ...
    // Read bt_list
    let rs_bt_list =
        try_get_optional_obj_list!("bt_list", bt_list, bt_list_len, ret_code, null_mut());
    ...
    match get_cert_data_hash(
        ...
        Err(e) => {
            log::error!("{:?}", format!("Error computing cert_data_hash: {:?}", e));
            *ret_code = CctpErrorCode::HashingError;
            null_mut()
        }
    }
...
impl Default for ZendooBatchProofVerifierResult {
    fn default() -> Self {
        Self {
            result: false,
            failing_proofs: null_mut(),
            num_failing_proofs: 0,
        }
    }
}

#[no_mangle]
pub extern "C" fn zendoo_free_batch_proof_verifier_result(
    raw_result: *mut ZendooBatchProofVerifierResult,
) {
    ...
    unsafe {
        ...
        Vec::from_raw_parts(
            (*result).failing_proofs,
            (*result).num_failing_proofs,
            (*result).num_failing_proofs,
        );
    };
}
...
pub extern "C" fn zendoo_add_certificate_proof_to_batch_verifier(
    ...
    let rs_batch_verifier =
        try_read_mut_raw_pointer!("batch_verifier", batch_verifier, ret_code, false);
    ...
    match rs_batch_verifier.add_zendoo_proof_verifier_data(
        ...
        Ok(()) => true,
        Err(e) => {
            log::error!("{:?}", format!("Error adding proof to the batch: {:?}", e));
...
pub extern "C" fn zendoo_batch_verify_all_proofs(
    ...
    match result {
        ...
        Err(e) => {
            log::error!("{:?}", format!("Batch proof verification failure: {:?}", e));
            match e {
...
pub extern "C" fn zendoo_init_poseidon_hash_constant_length(
    ...
    let personalization = try_read_optional_double_raw_pointer!(
        "personalization",
        personalization,
        personalization_len,
        ret_code,
    ...
    Box::into_raw(Box::new(get_poseidon_hash_constant_length(
        input_size,
        personalization,
    )))
}
...
pub extern "C" fn zendoo_init_poseidon_hash_variable_length(
    ...
    Box::into_raw(Box::new(get_poseidon_hash_variable_length(
        mod_rate,
        personalization,
    )))
}
...
pub extern "C" fn zendoo_update_poseidon_hash_from_raw(
    ...
    // Read input bytes
    let input_bytes =
        try_get_buffer_constant_size!("input_fe_bytes", fe, FIELD_SIZE, ret_code, false);
    ...
    match deserialize_from_buffer(input_bytes, None, None) {
        ...
        Err(e) => {
            *ret_code = CctpErrorCode::InvalidBufferData;
            log::error!("{:?}", format!("Error deserializing input: {:?}", e));
            false
        }
    }
}
...
pub extern "C" fn zendoo_finalize_poseidon_hash(
    ...
    match finalize_poseidon_hash(digest) {
        ...
        Err(e) => {
            log::error!("{:?}", format!("Error finalizing the hash: {:?}", e));
            *ret_code = CctpErrorCode::HashingError;
            null_mut()
        }
    }
...
#[no_mangle]
pub extern "C" fn zendoo_free_poseidon_hash(digest: *mut FieldHash) {
    free_pointer(digest)
}
...
pub extern "C" fn zendoo_new_ginger_mht(
    ...
    *ret_code = CctpErrorCode::OK;
    match new_ginger_mht(height, processing_step) {
        ...
        Err(e) => {
            log::error!("{:?}", format!("New merkle tree error: {:?}", e));
            *ret_code = CctpErrorCode::MerkleRootBuildError;
            null_mut()
        }
    }
    ...
    match append_leaf_to_ginger_mht(tree, &leaf) {
        ...
        Err(e) => {
            *ret_code = CctpErrorCode::MerkleTreeError;
            log::error!("{:?}", format!("Error appending leaf: {:?}", e));
            false
        }
    }
}
...
pub extern "C" fn zendoo_append_leaf_to_ginger_mht_from_raw(
    ...
    match deserialize_from_buffer(leaf_bytes, None, None) {
        ...
        Err(e) => {
            *ret_code = CctpErrorCode::InvalidBufferData;
            log::error!("{:?}", format!("Error deserializing leaf: {:?}", e));
            false
        }
    }
}
...
pub extern "C" fn zendoo_finalize_ginger_mht(
    ...
    match finalize_ginger_mht(tree) {
        Ok(tree_copy) => Box::into_raw(Box::new(tree_copy)),
        Err(e) => {
            log::error!("{:?}", format!("Finalize merkle tree error: {:?}", e));
            *ret_code = CctpErrorCode::MerkleTreeError;
            null_mut()
        }
    }
...
pub extern "C" fn zendoo_get_ginger_mht_root(
    ...
    match get_ginger_mht_root(tree) {
        ...
        None => {
            *ret_code = CctpErrorCode::MerkleRootBuildError;
            log::error!("{:?}", "Error: tree not finalized");
            null_mut()
        }
    }
}
...
#[no_mangle]
pub extern "C" fn zendoo_get_ginger_empty_node(height: usize) -> *mut FieldElement {
    let max_height = GINGER_MHT_POSEIDON_PARAMETERS.nodes.len() - 1;
    assert!(
        height <= max_height,
        "Empty node not pre-computed for height {}",
        height
    );
...
pub extern "C" fn zendoo_verify_ginger_merkle_path(
    ...
    match verify_ginger_merkle_path(path, height, leaf, root) {
        ...
        Err(e) => {
            *ret_code = CctpErrorCode::MerkleTreeError;
            log::error!("{:?}", format!("Error verifying merkle path: {:?}", e));
            false
        }
    }
}
...
#[no_mangle]
pub extern "C" fn zendoo_free_ginger_merkle_path(path: *mut GingerMHTPath) {
    free_pointer(path)
}
...
#[no_mangle]
pub extern "C" fn zendoo_free_ginger_mht(tree: *mut GingerMHT) {
    free_pointer(tree)
}
...
pub extern "C" fn zendoo_get_sc_pk_proving_system_type_from_buffer(
    ...
    let sc_pk_bytes = try_get_buffer_variable_size!(
        "sc_pk_buffer",
        sc_pk_bytes,
        ret_code,
        ProvingSystem::Undefined
...
#[cfg(feature = "mc-test-circuit")]
#[no_mangle]
pub extern "C" fn zendoo_sc_pk_free(sc_pk: *mut ZendooProverKey) {
    free_pointer(sc_pk)
}
...
fn _zendoo_generate_mc_test_params(
    ...
    match ps_type {
        ...
        ProvingSystem::Undefined => {
            log::error!("Error: Undefined proving system");
            *ret_code = CctpErrorCode::InvalidValue;
            return false;
        }
    }
    ...
    let params = match circ_type {
        TestCircuitType::Certificate => {
            ...
            mc_test_circuits::cert::generate_parameters(
                ps_type,
                num_constraints,
                true,
                segment_size,
        ...
        TestCircuitType::CSW => {
            ...
            mc_test_circuits::csw::generate_parameters(ps_type, num_constraints, true, segment_size)
        }
        TestCircuitType::CSWNoConstant => {
            params_path.push_str("csw_no_const_");
            mc_test_circuits::csw::generate_parameters(
                ps_type,
                num_constraints,
                false,
                segment_size,
        ...
        TestCircuitType::Undefined => {
            log::error!("Error: Undefined circuit type");
            *ret_code = CctpErrorCode::InvalidValue;
            return false;
        }
    };
    ...
    match params {
        Ok((pk, vk)) => {
            ...
            if pk_ser_res.is_err() {
                log::error!(
                    "{:?}",
                    format!("Error writing pk to file: {:?}", pk_ser_res.unwrap_err())
                );
            ...
            if vk_ser_res.is_err() {
                log::error!(
                    "{:?}",
                    format!("Error writing vk to file: {:?}", vk_ser_res.unwrap_err())
                );
                *ret_code = CctpErrorCode::InvalidFile;
                return false;
            }
        ...
        Err(e) => {
            log::error!("{:?}", format!("Error generating test params: {:?}", e));
            *ret_code = CctpErrorCode::GenericError;
            false
        }
    }
...
pub extern "C" fn zendoo_generate_mc_test_params(
    ...
    // Read params_dir
    let path_str =
        OsString::from_wide(unsafe { slice::from_raw_parts(params_dir, params_dir_len) });
    ...
    _zendoo_generate_mc_test_params(
        circ_type,
        ps_type,
        num_constraints,
        with_prev_hash,
...
fn _zendoo_create_cert_test_proof(
    ...
    // Read bt_list
    let rs_bt_list = try_get_optional_obj_list!(
        "bt_list",
        bt_list,
        bt_list_len,
        ret_code,
    ...
    // Read mandatory, constant size data
    let rs_sc_id = try_read_raw_pointer!(
        "sc_id",
        sc_id,
        ret_code,
        Err(ProvingSystemError::Other("".to_owned()))
    ...
    let rs_pk = try_read_raw_pointer!(
        "sc_pk",
        sc_pk,
        ret_code,
        Err(ProvingSystemError::Other("".to_owned()))
...
pub extern "C" fn zendoo_create_cert_test_proof(
    ...
    match _zendoo_create_cert_test_proof(
        ...
        Ok(proof) => {
            ...
            if proof_ser_res.is_err() {
                log::error!(
                    "{:?}",
                    format!(
                        "Error writing proof to file {:?}",
                        proof_ser_res.unwrap_err()
                    )
                );
                *ret_code = CctpErrorCode::InvalidFile;
                return false;
        ...
        Err(e) => {
            log::error!("{:?}", format!("Error creating proof {:?}", e));
...
fn _zendoo_create_csw_test_proof(
    ...
    let rs_nullifier = try_read_raw_pointer!(
        "nullifier",
        nullifier,
        ret_code,
        Err(ProvingSystemError::Other("".to_owned()))
    );
    let rs_cert_data_hash = try_read_optional_raw_pointer!(
        "cert_data_hash",
        cert_data_hash,
        ret_code,
        Err(ProvingSystemError::Other("".to_owned()))
    ...
    let rs_mc_pk_hash = try_get_buffer_constant_size!(
        "mc_pk_hash",
        mc_pk_hash,
        UINT_160_SIZE,
        ret_code,
...
pub extern "C" fn zendoo_sc_proof_assert_eq(
    ...
    match (proof_1, proof_2) {
        (
            ZendooProof::CoboundaryMarlin(cob_marlin_proof_1),
            ZendooProof::CoboundaryMarlin(cob_marlin_proof_2),
        ) => cob_marlin_proof_1 == cob_marlin_proof_2,
        (ZendooProof::Darlin(darlin_proof_1), ZendooProof::Darlin(darlin_proof_2)) => {
            darlin_proof_1 == darlin_proof_2
        }
        _ => false,
    }
}
